import { DollarSign, PieChart, TrendingUp, Users, Wallet } from 'lucide-react';
...
export default function Group() {
  ...
  const cards = [
    ...
    {
      title: 'Group Total Profit',
      value: group.totalProfit,
      icon: TrendingUp,
      color: group.totalProfit >= 0 ? 'text-success' : 'text-error',
    },
  ];
  ...
  return (
    <div className="font-body">
      ...
      <div className="bg-surface p-8 rounded-2xl border border-border shadow-luxury">
        ...
        <p className="text-text-muted mb-6">Seeded member profiles keep the showcase complete without exposing production records.</p>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {members.map((member) => (
            <div
              key={member.id}
              className="bg-surface-hover p-4 rounded-xl border border-border flex items-center gap-4 hover:bg-background transition-colors shadow-sm"
            >
